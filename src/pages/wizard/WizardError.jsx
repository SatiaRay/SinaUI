'use client';
import React from 'react';

const WizardError = ({ message, reset, defaultMessage }) => {
  return (
    <div className="bg-red-50 dark:bg-red-900/20 p-6 rounded-lg text-center shadow-md border border-red-200 dark:border-red-0">
      <p className="text-red-600 dark:text-red-400 text-sm md:text-base">
        {message || defaultMessage || 'خطایی رخ داده است.'}
      </p>
      {reset && (
        <button
          onClick={() => reset()}
          className="mt-3 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all"
        >
          تلاش مجدد
        </button>
      )}
    </div>
  );
};

export default WizardError;
