...
const ValidationError = ({ message }) => (
  <div className="p-4 mb-4 bg-amber-100 dark:bg-amber-900/30 border border-amber-300 dark:border-amber-700 rounded-lg flex items-start gap-3">
...
const LogSearchPage = () => {
  ...
  /**
   * Effect for handling 404 errors with SweetAlert
   */
  useEffect(() => {
    // فقط زمانی آلرت نشان داده شود که:
    // 1. خطا وجود داشته باشد
    // 2. خطا 404 باشد
    // 3. قبلا برای این شناسه آلرت نشان داده نشده باشد
    // 4. جستجو انجام شده باشد
    if (
      isError &&
      error?.status === 404 &&
      hasSearched &&
  ...
  }, [isError, error, logId, hasSearched, lastSearchedId]);
  ...
  return (
    <div className="flex flex-1 flex-col items-center overflow-y-auto scrollbar-hide bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-900 dark:to-gray-800 py-8 px-4 sm:px-6 lg:px-8 min-h-screen">
      {/* Main content */}
      <div className="w-full max-w-4xl">
        ...
        {/* Validation Error - Show only for validation errors */}
