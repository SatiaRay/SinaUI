 ...
 * @param {Array} items - List of integration objects
 * @param {boolean} loading - Global loading state for disabling actions
 * @param {string} selectedId - ID of currently selected integration
 * @param {function} onSelect - Callback when an integration is selected
 * @param {function} onDelete - Callback to delete integration by ID
 * @param {function} onEdit - Callback to open edit modal for integration
 */
export default function IntegrationsList({
  ...
  const rowBase = 'cursor-pointer transition';
  const selectedStyle =
  ...
  const actionBtnBase = 'px-3 py-2 rounded-xl transition disabled:opacity-60';
      ...
      {items.map((it) => {
        ...
        return (
          <div
            key={it.id}
            onClick={() => onSelect(it)}
            className={`rounded-2xl border p-4 ${rowBase} ${isSelected ? selectedStyle : normalStyle}`}
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <span className={statusBadge(it.isPublic)}>
            ...
            >
              <button
                disabled={loading}
                onClick={() => onEdit(it)}
                className={`${actionBtnBase} border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-800`}
              >
                ویرایش
              </button>
              <button
                ...
                className={`${actionBtnBase} bg-rose-600 text-white hover:bg-rose-700`}
              >
                حذف
              </button>
            </div>
          ...
          {items.map((it) => {
            ...
            return (
              <tr
                key={it.id}
                onClick={() => onSelect(it)}
                className={`${rowBase} ${isSelected ? 'bg-indigo-50 dark:bg-indigo-900/25' : 'hover:bg-gray-50 dark:hover:bg-gray-900/40'}`}
              >
