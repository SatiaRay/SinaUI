    ...
    setActiveJobs((prev) => {
      ...
      if (updatedJobs[jobId]) {
        updatedJobs[jobId] = {
          ...
          vectorizationProgress:
            data.status_info?.vectorization_batch &&
            updatedJobs[jobId].vectorizationProgress !== null
              ? data.status_info.vectorization_batch.progress.progress_percent
              : updatedJobs[jobId].vectorizationProgress,
          totalUrls: data.progress?.total_urls || updatedJobs[jobId].totalUrls,
          crawledUrls:
            data.progress?.crawled_urls || updatedJobs[jobId].crawledUrls,
          exceptionUrls:
            data.progress?.exception_urls || updatedJobs[jobId].exceptionUrls,
        };
      }
      return updatedJobs;
    });
          ...
          <div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label
                  htmlFor="store-in-vector"
                  className="text-sm text-gray-700 dark:text-gray-300 select-none cursor-pointer"
                >
                  اسناد ساخته شده پس از خزش فعال شوند و در دسترس هوش مصنوعی قرار
                  گیرند.
                </label>
              </div>
            </div>
                ...
                {Object.entries(activeJobs).map(([jobId, job]) => (
                  ...
                  >
                    <div className="flex justify-between items-center mb-3">
                      <h4 className="font-medium text-gray-900 dark:text-white truncate max-w-md">
                        {job.url}
                      </h4>
                      <span
