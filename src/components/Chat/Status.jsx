...
const Status = () => {
  ...
  const API_BASE_URL =
    process.env.REACT_APP_CHAT_API_URL || 'https://khan2.satia.co:1011';
  ...
  const fetchJobs = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/crawl/jobs?active=${filter === 'active'}`
      );
          ...
          newJobs[job.job_id] = {
            ...
            statusMsg:
              job.status.toLowerCase() === 'queued'
                ? 'در صف'
                : job.status.toLowerCase() === 'started'
                  ? 'در حال خزش'
    ...
    setActiveJobs((prev) => {
      ...
      return {
        ...
        [jobId]: {
          ...
          statusMsg:
            data.status_info?.msg === 'saving data'
              ? 'ذخیره در پایگاه داده'
              : data.status_info?.msg === 'running crawl'
                ? 'در حال خزش'
                : data.status_info?.msg === 'Queued'
                  ? 'در صف'
                  : data.status?.toLowerCase() === 'finished'
                    ? 'تکمیل شده'
                    : data.status || prev[jobId].statusMsg,
          crawlProgress: data.status_info?.progress?.progress_percent || 0,
          vectorizationProgress:
            data.status_info?.vectorization_batch &&
            prev[jobId].vectorizationProgress !== null
              ? data.status_info.vectorization_batch.progress
                  .progress_percent || 0
          ...
          isSavingData:
            data.status_info?.msg === 'saving data' || prev[jobId].isSavingData,
        },
      };
    });
  };
    ...
    setActiveJobs((prev) => {
      ...
      return {
        ...
        [jobId]: {
          ...
          crawledUrls:
            data.progress?.crawled_urls || prev[jobId].crawledUrls || 0,
          exceptionUrls:
            data.progress?.exception_urls || prev[jobId].exceptionUrls || 0,
        ...
        <div className="space-y-6 w-full">
            ...
            <button
              ...
              className={`px-4 py-2 max-md:w-1/2 rounded-lg font-medium transition-all duration-200 ${
                filter === 'active'
                  ...
                  : 'bg-gray-300 dark:bg-gray-700 dark:text-white hover:bg-gray-400 dark:hover:bg-gray-600'
              }`}
            >
              در حال اجرا
            </button>
          ...
          {Object.entries(activeJobs).length > 0 ? (
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                          شروع: {formatDate(job.started_at)}{' '}
                          {job.recursive && '(بازگشتی)'}
                        </p>
                      </div>
                      <span
                        className={`text-sm px-3 py-1 rounded-full flex items-center gap-2 ${
                          job.status === 'finished'
                            ...
                            : job.statusMsg === 'ذخیره در پایگاه داده'
                      ...
                      >
                        ...
                        {job.statusMsg === 'در حال خزش' && (
                          <ArrowPathIcon className="h-4 w-4 animate-spin" />
                        )}
                        {job.statusMsg === 'ذخیره در پایگاه داده' && (
                          <CloudArrowDownIcon className="h-4 w-4" />
                        )}
                        {job.statusMsg === 'تکمیل شده' && (
            ...
            <p className="text-gray-600 dark:text-gray-400 text-center">
              {filter === 'active'
                ? 'هیچ پردازشی در حال اجرا نیست.'
                : 'هیچ پردازش تکمیل‌شده‌ای یافت نشد.'}
            </p>
          )}
          {selectedJobId && (
            <JobDetailsModal
